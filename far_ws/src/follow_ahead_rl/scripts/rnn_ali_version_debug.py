...
class RNN(nn.Module):
    ...
    def forward(self, obs, act, hid):
        ...
        print('enter into self.rnn(x, hid)', hid)
        h, c = self.rnn(x, hid)
        print('exit from self.rnn(x, hid)')
        mu = self.mu(h)
        # print("what is mu", mu)
...
class RolloutGenerator(object):
    ...
    def get_action_and_transition(self, hidden, seq_len=1600, render=False):
        ...
        print('enter into self.model.forward(obs, act, hid)')
        _, _, hid = self.model.forward(obs, act, hid)
        print('exit from self.model.forward(obs, act, hid)')
        m = nn.Softmax(dim=1)
        act = m(act) 
    ...
    def rollout(self, params, render=False):
        ...
        i = 0
        while True:
            print('enter into self.get_action_and_transition(hidden)')
            action, hidden = self.get_action_and_transition(hidden)
            print('exit from self.get_action_and_transition(hidden)')
    ...
    with torch.no_grad():
        ...
        while e_queue.empty():
            ...
            #   print("we are in else statement")
              ...
              print("we are putting stuff in r_queue", s_id)
