...
class MCTS(nn.Module):
    def __init__(self, env, obs_size, hidden_size, n_actions, gamma = 1.0, start_epsilon = 1.0, end_epsilon = 0.05, epsilon_decay = 0.99999):
        ...
        self.N = defaultdict(lambda: np.zeros(self.n_action)) #sum of expected rewards
        self.returns_sum = defaultdict(lambda: np.zeros(self.n_action)) #number of actions performed
...
#     #update policy to reflect q values
    ...
    def reinforce_mc(self, env, num_episodes, gamma=1.0, update_every=1):
        ...
        for i_episode in range(1, num_episodes+1):
            ...
            if  i_episode % int(update_every * num_episodes - 1) ==0:
...
timer = np.arange(0, int(len(reward[0])), 1)

print(list(reward[0]))

plt.plot(timer,reward[0])
